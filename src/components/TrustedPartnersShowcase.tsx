  ...
  return (
    <div className="relative w-full">
      ...
      <div className="glass-card rounded-3xl p-5 sm:p-7 md:p-8 shadow-2xl border border-purple-400/25 relative overflow-hidden backdrop-blur-xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 mb-6 border-b border-white/10">
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm text-white/60">
              Trusted by tech media companies, software clans, design studios, and automation agencies to scale their Fiverr presence.
            </p>
          </div>
        ...
        <div className="relative w-full overflow-hidden py-3 mb-8">
          ...
          <div className="flex gap-4 w-max animate-marquee hover:[animation-play-state:paused]">
            {[...PARTNER_COMPANIES, ...PARTNER_COMPANIES].map((company, index) => (
              <div
                ...
                className="w-56 sm:w-64 h-24 sm:h-28 px-5 py-3 rounded-2xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-orange-400/40 transition-all duration-300 flex items-center justify-center cursor-pointer group shadow-lg flex-shrink-0"
              >
        ...
        {/* Interactive Highlighted Companies Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3.5 pt-2">
          {PARTNER_COMPANIES.map((company) => (
            <div
              key={company.id}
              onMouseEnter={() => setHoveredPartner(company.id)}
              onMouseLeave={() => setHoveredPartner(null)}
              className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col justify-between relative group ${
                hoveredPartner === company.id
                  ? 'bg-white/10 border-orange-400/60 shadow-xl -translate-y-1'
                  : 'bg-white/[0.03] border-white/10 hover:border-white/20'
              }`}
            >
              {/* Top Logo Container */}
              <div className="h-16 w-full flex items-center justify-start p-1 mb-3 rounded-xl bg-black/40 border border-white/5">
                {company.svgLogo}
              </div>
              ...
              <div className="space-y-1.5 mb-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="flex items-center gap-1 text-[10px] text-amber-400 font-bold px-1.5 py-0.5 rounded-full bg-amber-400/10 border border-amber-400/20">
              ...
              <div className="pt-3 border-t border-white/10 flex flex-col gap-1 text-xs">
                <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold border text-center ${company.badgeBg}`}>
                  {company.growthTag}
                </span>
                <span className="text-[9px] text-white/40 font-semibold uppercase tracking-wider text-center">
                  {company.industry}
                </span>
              </div>
            </div>
        ...
        <div className="mt-6 pt-4 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/60">
          <div className="flex items-center gap-2">
            <div className="flex -space-x-1.5">
              <div className="w-6 h-6 rounded-full bg-blue-500 border border-white/20 flex items-center justify-center text-[10px] font-bold text-white">HA</div>
              <div className="w-6 h-6 rounded-full bg-rose-500 border border-white/20 flex items-center justify-center text-[10px] font-bold text-white">GG</div>
              <div className="w-6 h-6 rounded-full bg-sky-500 border border-white/20 flex items-center justify-center text-[10px] font-bold text-white">KT</div>
              <div className="w-6 h-6 rounded-full bg-purple-500 border border-white/20 flex items-center justify-center text-[10px] font-bold text-white">CR</div>
              <div className="w-6 h-6 rounded-full bg-emerald-500 border border-white/20 flex items-center justify-center text-[10px] font-bold text-white">WB</div>
            </div>
            <span>Trusted across Tech, Design, Software & Automations</span>
          </div>

          <div className="flex items-center gap-4 text-[11px] font-semibold text-white/80">
            <span className="flex items-center gap-1 text-emerald-400">
              <CheckCircle2 className="w-3.5 h-3.5" /> High-Intent Keyword Indexing
            </span>
            <span className="hidden sm:flex items-center gap-1 text-amber-400">
              <Star className="w-3.5 h-3.5 fill-amber-400" /> 100% Client Satisfaction
            </span>
          </div>
        </div>
      </div>
